...
(async () => {
  // R27.1a — surface email transport configuration at boot so it's obvious from the
  // Render logs whether order/OTP/welcome mails will actually send.
  console.log(`[boot] brevo configured: ${process.env.BREVO_SMTP_KEY ? "yes" : "no"}`);
  ...
  try {
    ...
    runR26_4bMigrations();
    console.log("[boot] step: post-R26.4b migrations");
    runR26_5Migrations();
    console.log("[boot] step: post-R26.5 migrations");
    runR26_6aMigrations();
    console.log("[boot] step: post-R26.6a migrations");
    runR26_6bMigrations();
    console.log("[boot] step: post-R26.6b migrations");
    runR26_6cMigrations();
    console.log("[boot] step: post-R26.6c migrations");
    runR26_6dMigrations();
    console.log("[boot] step: post-R26.6d migrations");
    runR26_6eMigrations();
    console.log("[boot] step: post-R26.6e migrations");
    runR26_6gMigrations();
    console.log("[boot] step: post-R26.6g migrations");
    runR26_6iMigrations();
    console.log("[boot] step: post-R26.6i migrations");
    runR26_6jMigrations();
    console.log("[boot] step: post-R26.6j migrations");
    runR26_6kMigrations();
    console.log("[boot] step: post-R26.6k migrations");
    runR26_6lMigrations();
    console.log("[boot] step: post-R26.6l migrations");
    runR27_0Migrations();
    console.log("[boot] step: post-R27.0 migrations");
    runR27_1Migrations();
    console.log("[boot] step: post-R27.1 migrations");
    runR27_1aMigrations();
    console.log("[boot] step: post-R27.1a migrations");
    runR27_2Migrations();
    console.log("[boot] step: post-R27.2 migrations");
    runR27_3Migrations();
    console.log("[boot] step: post-R27.3 migrations");
    runR27_4Migrations();
    ...
    runR27_7Migrations();
    console.log("[boot] step: post-R27.7 migrations");
    runR27_8Migrations();
    console.log("[boot] step: post-R27.8 migrations");
    runR27_9Migrations();
    console.log("[boot] step: post-R27.9 migrations");
    runR27_10Migrations();
    console.log("[boot] step: post-R27.10 migrations");
    runR27_11Migrations();
    console.log("[boot] step: post-R27.11 migrations");
    runR27_12Migrations();
    console.log("[boot] step: post-R27.12 migrations");
    runR27_13Migrations();
    console.log("[boot] step: post-R27.13 migrations");
    runR27_14Migrations();
    // R27.14 — reconcile branch_stock.dispatched_qty from dispatch invoice items so
    // already-dispatched stock stops appearing under the in_stock filter. Idempotent.
    try {
      const r27mod = await import("./storage-r27");
      r27mod.backfillDispatchedQty();
    } catch (e: any) { console.error("[R27.14] boot backfill failed:", e?.message || e); }
    ...
    const { runR27_32Migrations, runR27_33Migrations, runR27_33aMigrations, runR27_34aMigrations, runR27_34bMigrations, runR27_35Migrations, runR27_36Migrations, runR27_36aMigrations, runR27_36bMigrations, runR27_36aDataMigration, runR28_1Migrations, runR28_2Migrations, runR28_3Migrations, runR28_4Migrations, runR28_5Migrations, runR28_6Migrations } = await import("./migrations");
    runR27_32Migrations();
    ...
    runR27_34bMigrations();
    console.log("[boot] step: post-R27.34b currency-change audit columns + search indexes");
    runR27_35Migrations();
    console.log("[boot] step: post-R27.35 payment approval columns + retroactive backfill");
    runR27_36Migrations();
    console.log("[boot] step: post-R27.36 expense category/payee/expense tables + indexes");
    runR27_36aMigrations();
    console.log("[boot] step: post-R27.36a unified expense columns + reconciliation/counter tables");
    runR27_36bMigrations();
    console.log("[boot] step: post-R27.36b payment-model columns (paid_from + handled_by_staff)");
    runR27_36aDataMigration();
    console.log("[boot] step: post-R27.36a legacy R27.6 expense backfill");
    ...
    // R28 Session 3 — auto_publish_log + part_image_cache (additive, idempotent).
    ...
    // R28.6 — quote_otp + quote_request (additive, idempotent). Backs the
    // public /get-quote wizard (OTP challenge + final RFQ submission).
    // Runs whether the flow is enabled or not so audit rows can be written
    // the moment the endpoints go live.
    runR28_6Migrations();
    console.log("[boot] step: post-R28.6 quote_otp + quote_request tables");
...
<p>These vendor-assigned items have not been collected. Please follow up.</p>`;
  ...
  // ---- R27.36 Recurring expense drafts — 02:00 IST daily ----
  try {
    const { startExpenseRecurringCron } = await import("./expense-recurring-cron");
    const { rawSqlite } = await import("./storage");
    startExpenseRecurringCron(rawSqlite);
  } catch (e: any) {
