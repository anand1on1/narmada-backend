// R27.32 frontend — Process Payment dashboard (vendor payment slips + payment queue).
// UI for the /api/payments/* backend shipped in 74f4048. Two tabs:
...
//      of one JPG slip per vendor.
//   2. Assign Payments — the per-vendor payment queue: download slip, mark paid (with
//      optional proof upload), mark skipped, or bulk mark paid.
// Role-gated to admin | procurement | finance (matches the backend requireRole guard).
// All HTTP goes through adminFetch so the x-admin-token header is attached.
...
export default function AdminProcessPayment() {
  ...
  const allowed = PAYMENT_ROLES.includes((role as string) || "");
...
function ProcessVendorsTab({ onGenerated }: { onGenerated: () => void }) {
  ...
  async function loadPos() {
    ...
    try {
      ...
      const r = await adminFetch(token, `/api/payments/pos?${p}`);
  ...
  async function continueToAggregate() {
    ...
    setAggregating(true);
    try {
      const r = await adminFetch(token, `/api/payments/aggregate`, {
        method: "POST",
  ...
  async function generate() {
    ...
    setGenerating(true);
    try {
      // R27.32a — only checked items are sent; vendors left with no payable items drop out.
      ...
      toast({
        ...
        description:
          ...
          (approval === "pending_approval"
            ? " Over ₹5,000 — sent to Payment Approvals for release by narmadamobility123."
            : ""),
      });
...
function AssignPaymentsTab() {
  ...
  async function load() {
    ...
    try {
      ...
      const r = await adminFetch(token, `/api/payments/batches?${p}`);
      if (!r.ok) throw new Error((await r.json().catch(() => ({}))).error || `HTTP ${r.status}`);
      const d = await r.json();
  ...
  // R27.35 — admins follow the same threshold as everyone else, so the queue locks
  // here too until narmadamobility123 releases the batch.
  const batchLocked =
  ...
  async function downloadSlip(v: BatchVendorRow) {
    if (!token) return;
    try {
      const r = await adminFetch(token, `/api/payments/batches/${v.id}/slip`);
  ...
  async function uploadProof(file: File): Promise<string | null> {
    ...
    // Direct fetch: FormData must not carry a JSON Content-Type (adminFetch would set one).
  ...
  async function confirmMarkPaid() {
    ...
    try {
      ...
      const r = await adminFetch(token, `/api/payments/batches/${paidTarget.id}/mark-paid`, {
        method: "POST",
  ...
  async function confirmMarkSkipped() {
    ...
    setSubmitting(true);
    try {
      const r = await adminFetch(token, `/api/payments/batches/${skipTarget.id}/mark-skipped`, {
        method: "POST",
  ...
  async function confirmBulkPaid() {
    ...
    setSubmitting(true);
    try {
      const r = await adminFetch(token, `/api/payments/batches/bulk-mark-paid`, {
        method: "POST",
            ...
            <div className="overflow-x-auto">
              {selectedBatch.approval_status === "pending_approval" && (
                <div className="m-4 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900" data-testid="banner-approval-pending">
                  <strong>Slip pending approval.</strong> Payments can be marked paid once narmadamobility123 approves this batch.
                </div>
              )}
              {selectedBatch.approval_status === "rejected" && (
                <div className="m-4 rounded-lg border border-rose-300 bg-rose-50 px-4 py-3 text-sm text-rose-900" data-testid="banner-approval-rejected">
                  <strong>Slip rejected:</strong> {selectedBatch.rejection_reason || "no reason given"}. Regenerate the slip with corrections.
                </div>
              )}
              <table className="w-full text-sm">
                <thead>
                      ...
                      <td className="px-3 py-3">
                        <div className="flex items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            title={batchLocked ? "Awaiting approval from narmadamobility123" : undefined}
