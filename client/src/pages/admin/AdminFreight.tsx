...
export default function AdminFreight() {
  ...
  async function addNew(e: React.FormEvent) {
    ...
    const r = await adminFetch(token, `/api/admin/freight-charges/${encodeURIComponent(newPart.trim())}`, {
      method: "PATCH",
      body: JSON.stringify({
        freight_inr: Number(newFreight) || 0,
        source: newSource.trim() || undefined,
        destination: newDest.trim() || undefined,
        mode: newMode.trim() || undefined,
      }),
    });
    if (r.ok) { toast({ title: "Added", description: newPart }); setNewPart(""); setNewFreight(""); setNewSource(""); setNewDest(""); setNewMode(""); load(); }
  }
    ...
    <AdminLayout title="Freight Charges">
      <div className="flex flex-wrap items-center gap-2 mb-4">
        <form onSubmit={(e) => { e.preventDefault(); load(); }} className="flex gap-2">
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search part #, product, city, source, destination or mode" className="px-3 py-1.5 rounded-lg border bg-card text-sm w-80" data-testid="search-input" />
      ...
      <form onSubmit={addNew} className="flex flex-wrap gap-2 mb-6 items-end bg-card border rounded-xl p-4">
        <div><label className="text-xs text-muted-foreground">Part Number</label><input value={newPart} onChange={(e) => setNewPart(e.target.value)} className="block px-3 py-1.5 rounded-lg border bg-background text-sm w-40" data-testid="new-part" /></div>
        <div><label className="text-xs text-muted-foreground">Source</label><input value={newSource} onChange={(e) => setNewSource(e.target.value)} placeholder="e.g. Delhi" className="block px-3 py-1.5 rounded-lg border bg-background text-sm w-32" data-testid="new-source" /></div>
        <div><label className="text-xs text-muted-foreground">Destination</label><input value={newDest} onChange={(e) => setNewDest(e.target.value)} placeholder="e.g. Patna" className="block px-3 py-1.5 rounded-lg border bg-background text-sm w-32" data-testid="new-dest" /></div>
        <div><label className="text-xs text-muted-foreground">Mode</label><input value={newMode} onChange={(e) => setNewMode(e.target.value)} placeholder="Road / Air" className="block px-3 py-1.5 rounded-lg border bg-background text-sm w-28" data-testid="new-mode" /></div>
        <div><label className="text-xs text-muted-foreground">Freight (₹)</label><input type="number" value={newFreight} onChange={(e) => setNewFreight(e.target.value)} className="block px-3 py-1.5 rounded-lg border bg-background text-sm w-28" data-testid="new-freight" /></div>
      ...
      <div className="bg-card border rounded-xl overflow-hidden">
        {rows.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/50 text-left"><tr><th className="p-3">Part #</th><th className="p-3">Product</th><th className="p-3">Route</th><th className="p-3">Freight (₹)</th><th className="p-3"></th></tr></thead>
            <tbody className="divide-y">
              {rows.map((r) => (
                <tr key={r.id} className="hover:bg-muted/30" data-testid={`freight-row-${r.id}`}>
                  ...
                  <td className="p-3 text-muted-foreground text-xs">{[r.source, r.destination].filter(Boolean).join(" → ") || "—"}{r.mode ? ` · ${r.mode}` : ""}</td>
                  <td className="p-3">
